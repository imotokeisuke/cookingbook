...
async function loadDishes() {
  try {
    const res = await fetch(`${FB_BASE}/dishes.json`);
    const data = await res.json();
    if (!data) return [];
    const arr = Array.isArray(data) ? data : Object.values(data);
    return arr.filter(x => x !== null && typeof x === "object");
...
async function saveDishes(dishes) {
  if (!Array.isArray(dishes)) {
    console.error("saveDishes: data is not an array", dishes);
    return;
  }
  const isValid = dishes.every(d => d && typeof d === "object" && d.id && d.name);
  if (!isValid) {
    console.error("saveDishes: invalid dish object found", dishes);
    return;
  }
  try {
    await fetch(`${FB_BASE}/dishes.json`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
...
async function loadCalendarRecords() {
  try {
    const res = await fetch(`${FB_BASE}/calendarRecords.json`);
    const data = await res.json();
    if (!data) return [];
    const arr = Array.isArray(data) ? data : Object.values(data);
    return arr.filter(x => x !== null && typeof x === "object");
...
async function saveCalendarRecords(records) {
  if (!Array.isArray(records)) {
    console.error("saveCalendarRecords: data is not an array", records);
    return;
  }
  const isValid = records.every(r => r && typeof r === "object" && r.id && r.dishId);
  if (!isValid) {
    console.error("saveCalendarRecords: invalid record object found", records);
    return;
  }
  try {
    await fetch(`${FB_BASE}/calendarRecords.json`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(records)
    });
...
export default function App() {
  ...
  const emptyForm = { date: toDateStr(new Date()), photo: null, name: "", rating: 3, comment: "", url: "" };
  const [form, setForm] = useState(emptyForm);
  const fileRef = useRef();

  const persistDishes = async (next) => { setDishes(next); await saveDishes(next); };
  const persistCalendarRecords = async (next) => { setCalendarRecords(next); await saveCalendarRecords(next); };

  useEffect(() => {
    Promise.all([loadDishes(), loadCalendarRecords(), loadRecords(), loadTitle()]).then(([d, cr, oldRecs, t]) => {
      setTitle(t);
      if (d.length === 0 && oldRecs && oldRecs.length > 0) {
        const migratedDishes = [];
        const migratedCalendarRecords = [];
        oldRecs.forEach(r => {
          if (!r || !r.name || !r.id) return;
          let dish = migratedDishes.find(item => item.name === r.name);
          if (!dish) {
            dish = {
              id: "dish_" + r.id,
              name: r.name,
            ...
            migratedDishes.push(dish);
          }
          migratedCalendarRecords.push({
            id: r.id,
            dishId: dish.id,
            date: r.date,
            rating: r.rating,
        ...
        persistDishes(migratedDishes);
        persistCalendarRecords(migratedCalendarRecords);
      } else {
        setDishes(d);
        setCalendarRecords(cr);
      }
      setLoaded(true);
    });
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    let dish = dishes.find(d => d.name.trim() === form.name.trim());
    ...
    if (!dish) {
      ...
      await persistDishes(updatedDishes);
    }
    ...
    await persistCalendarRecords([...calendarRecords, recordEntry]);
    setForm(emptyForm);
    showToast("記録しました 🌸");
    setTab("zukan");
    setDetailId(dish.id);
  ...
  const handleEditSubmit = async () => {
    ...
    const updated = dishes.map(d => d.id === editForm.id ? editForm : d);
    await persistDishes(updated);
  ...
  const handleRemoveRecord = async (id) => {
    ...
    const next = calendarRecords.filter(r => r.id !== id);
    await persistCalendarRecords(next);
    showToast("削除しました 🗑");
  };
  ...
  const detail = dishes.find(d => d.id === detailId);
        ...
        <div style={{ padding: "16px 16px 0" }}>
          ...
          {tab === "calendar" && (
            <div>
              ...
              {weekDays.map((date, i) => {
                const dayRecs = calendarRecords.filter(r => r.date === date);
                    ...
                    <div style={{ flex: 1, minWidth: 0, paddingBottom: 4 }}>
                      {dayRecs.length === 0 ? (
                        ...
                        dayRecs.map(r => {
                          ...
                          const name = dish ? dish.name : "不明な料理";
                          const photo = dish ? dish.photo : null;
                          return (
                            <div
                              key={r.id}
                              onClick={() => { if (dish) { setDetailId(dish.id); setTab("zukan"); setIsEditingDetail(false); } }}
                              style={{
                            ...
                            >
                              {photo ? (
                                <img src={photo} alt={name} style={{ width: 50, height: 50, borderRadius: 12, objectFit: "cover", flexShrink: 0 }} />
                              ) : (
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ fontSize: 15, fontWeight: 700, color: C.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{name}</div>
            ...
            <div>
              {detail ? (
                isEditingDetail ? (
                  ...
                  <div>
                    ...
                    <div style={{ background: C.card, borderRadius: 22, border: `1.5px solid ${C.border}`, padding: 20 }}>
                      ...
                      <div style={{ marginBottom: 22 }}>
                        <label style={labelStyle}>レシピURL</label>
                        <input
                          type="url"
                          value={editForm.url || ""}
                  ...
                  const history = calendarRecords.filter(cr => cr.dishId === detail.id).sort((a, b) => b.date.localeCompare(a.date));
                  const timesMade = history.length;
                  const latestDate = timesMade > 0 ? history[0].date : null;
                  return (
                    /* Detail */
                    <div>
                      <button
                      ...
                      <div style={{ background: C.card, borderRadius: 22, border: `1.5px solid ${C.border}`, overflow: "hidden" }}>
                        <div style={{ padding: "18px 18px 0" }}>
                          ...
                          <div style={{ fontSize: 12, color: C.muted, marginTop: 8, marginBottom: 14 }}>
                            作った回数: {timesMade}回 {latestDate ? ` (最後に作った日: ${fmtDate(latestDate)})` : ""}
                          </div>
                        </div>
                        {detail.photo ? (
                        ...
                        <div style={{ padding: 18 }}>
                          {detail.comment ? (
                            <p style={{ fontSize: 15, color: C.text, lineHeight: 1.75, margin: "0 0 12px" }}>{detail.comment}</p>
                          ) : (
                            <p style={{ fontSize: 14, color: C.muted, margin: "0 0 12px" }}>コメントなし</p>
                          )}
                          {detail.url ? (
                            <a
                              ...
                              style={{
                                ...
                                fontSize: 13, color: C.accentDeep,
                            ...
                            >
                              ...
                              <span style={{ flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{detail.url}</span>
                              <span style={{ fontSize: 11, opacity: 0.7, flexShrink: 0 }}>↗</span>
                            </a>
                          ) : null}
                          
                          {timesMade > 0 && (
                            <div style={{ marginTop: 0, marginBottom: 20, borderTop: `1.5px solid ${C.border}`, paddingTop: 15 }}>
                              <div style={{ fontSize: 12, fontWeight: "bold", color: C.muted, marginBottom: 8 }}>調理履歴:</div>
                              <div style={{ display: "flex", gap: 6, flexWrap: "wrap", maxHeight: 80, overflowY: "auto" }}>
                                {history.map(h => (
                                  <span key={h.id} style={{ fontSize: 11, background: C.accentLight, color: C.accentDeep, padding: "3px 8px", borderRadius: 12 }}>
                                    {h.date.replace(/-/g, "/")}
                                  </span>
                                ))}
                              </div>
